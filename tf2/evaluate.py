...
def plot_model(x_test, compression_ratios, snr_lst, title, x_lablel, y_label):
    ...
    for snr in snr_lst:
        ...
        model_dic = EvaluateModel(
            x_test, compression_ratios, snr, mode='multiple')
        history.append(model_dic)
        label = 'Deep JSCC (SNR={0}dB)'.format(snr)
        plt.plot(compression_ratios,
                 model_dic['PSNR'], ls='--', c=colors[i], marker=markers[i], label=label)
        i += 1
        plt.title(title)
        plt.xlabel(x_lablel)
        plt.ylabel(y_label)
...
snr_lst = [20]
history = plot_model(x_test, compression_ratios, snr_lst,
                     title='AWGN Channel', x_lablel='k/n', y_label='PSNR (dB)')
...
for epoch in range(50, epoch_range, epoch_step):
    tf.keras.backend.clear_session()
    path = './CKPT_ByEpochs/CompRatio_{0}SNR{1}/Autoencoder_Epoch_{2}.h5'.format(
    	comp_ratio, snr, epoch)
    autoencoder = load_model(
    	path, custom_objects={'NormalizationNoise': NormalizationNoise})
    K.set_value(autoencoder.get_layer('normalization_noise_1').snr_db, snr)
    ...
    preds['SSIM'].append(structural_similarity(
    	true_img, pred_images, multichannel=True))
    preds['Epoch'].append(epoch)
...
for j in range((len(preds['Image'])+2)//plt_step):
    ...
    if i % col == 0:
        ...
        plt.title(str(int(preds['PSNR'][i])) +
                  '/{0:.3f}'.format(preds['SSIM'][i]))
    i = i+plt_step
